...
def condition_to_parameter(condition: dict) -> dict:
    ...
    def generate_regime_securite_sociale_parameter(
        ...
        data: dict = {condition_type: create_regime_dict(constraint)}
...
def convert_benefit_conditions_to_parameters(benefit: dict) -> ParameterNode:
    ...
    node_data.update(meta_donnees)

    profils: list[dict] = benefit.get("profils")
    if profils:
        profil_dict = profils_to_node_data(profils)
